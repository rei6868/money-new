...
function buildTotalBackFormula(txn) {
  const hasPercentBack = typeof txn.percentBack === 'number' && txn.percentBack > 0;
  const hasFixedBack = typeof txn.fixedBack === 'number' && txn.fixedBack > 0;

  if (!hasPercentBack || !hasFixedBack) {
    return '';
  }

  const amountLabel = formatFormulaNumber(txn.amount);
  const percentLabel = formatFormulaNumber(txn.percentBack);
  const fixedLabel = formatFormulaNumber(txn.fixedBack);

  if (!amountLabel || !percentLabel || !fixedLabel) {
    return '';
  }

  return `${amountLabel}*${percentLabel}% + ${fixedLabel}`;
}
...
function enrichTransaction(txn) {
  ...
  return {
    ...
    totalBack,
    finalPrice,
    totalBackFormula: buildTotalBackFormula(txn),
  };
}
